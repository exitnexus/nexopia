...
YAHOO.util.Event.on('preview_zoom', 'click', function()
{
	...
	else
	{
		...
		var goalWidth = Math.min(700, viewWidth - 50);
		var goalHeight = Math.min(600, viewHeight - 50);
		var goalLeft = Math.round((viewWidth - goalWidth) / 2);
		var goalTop = Math.round((viewHeight - goalHeight) / 2);
		
		var attributes = {
			points: { to: [goalLeft, goalTop + YAHOO.util.Dom.getDocumentScrollTop()] },
		...
		var shadowFixAttributes = {
			...
			width: { to: YAHOO.util.Dom.getViewportWidth() }
		};
