...
*         "zIndex": the base z-index to use.
*
*/
YAHOO.widget.ImageCropper = function ( img, config ) {
	...
	* @private
	*/
	function _setGeometry ( el, x, y, w, h, r ) {
		var p, d;
	...
	*/
	function _init () {
		...
		var mask, x, y, w, h, r, p, dd, minW, minH;
...
*/		
		_outerElem.style.width = img.clientWidth + 2 + "px";
		_outerElem.style.height = img.clientHeight + 2 + "px";
...
*/
		...
		if ( config ) {
			_cropElem.style.zIndex = config.zIndex;
			w = config.w;
			h = config.h;
			x = config.x;
			y = config.y;
		...
		if (YL.isNumber(minW)) {
			minW = Math.min(img.clientWidth, minW);
			if (YL.isNumber(r)) {
				minW = Math.min(img.clientHeight/r, minW);
			}
			w = Math.max(minW, w);
			if (YL.isNumber(r)) {
				h = w*r;
		...
		if (YL.isNumber(minH)) {
			minH = Math.min(img.clientHeight, minH);
			if (YL.isNumber(r)) {
				minH = Math.min(img.clientWidth*r, minH);
			}
			h = Math.max(minH, h);
			if (YL.isNumber(r)) {
				w = h/r;
		...
		_setGeometry( _cropElem, x, y, w, h, r );
			...
			dd.onDrag = function ( evt ) {
				...
				if ( YD.hasClass( hook, "l" ) ) {
					if ( dx < 0 ) {
						...
						dx = Math.min( (obj.w-config.minW), dx );
					}
					x = obj.x + dx;
					w = obj.w - dx;
				} else if ( YD.hasClass( hook, "r" ) ) {
					if ( dx < 0 ) {
						dx = Math.max( -(obj.w-config.minW), dx );
					} else {
				...
				if ( YD.hasClass( hook, "t" ) ) {
					if ( dy < 0 ) {
						...
						dy = Math.min( (obj.h-config.minH), dy );
					}
					y = obj.y + dy;
					h = obj.h - dy;
				} else if ( YD.hasClass( hook, "b" ) ) {
					if ( dy < 0 ) {
						dy = Math.max( -(obj.h-config.minH), dy );
					} else {
				...
				if ( YL.isNumber( config.xyratio ) && config.xyratio > 0 ) {
					// Handle the constraints.
					r = config.xyratio;
					h = Math.max(config.minH, Math.floor( w * r ));
					//Adjust x position if dragging from the left
					if (YD.hasClass(hook, "l")) {
						x -= h/r-w;
					}
					w = Math.floor(h/r);
	...
	*/
	...
	this.getCropRegionNormalized = function () {
		var crop = _getGeometry( _cropElem );
		var size = _getGeometry( img );
		crop.x /= size.w;
		crop.y /= size.h;
		crop.w /= size.w;
		crop.h /= size.h;
