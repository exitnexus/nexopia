...
module Gallery
	class Gallery < Storable
		...
		def collapsable_bucket(event)
			return [Gallery::typeid, owner.userid, self, event.eventtype]
		end
		...
		def self.get_by_id(uid, id)
			return Gallery.find(:first, [uid, id], :promise);	
		end
...
class User < Cacheable
	relation_multi_cached :galleries, :userid, Gallery::Gallery, "ruby_galleries"
end
