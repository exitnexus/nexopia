lib_require :Core, "storable/relation_count"

describe RelationCount do
	before do
	...
	it "should look for the ids in memcache" do
		@relation = RelationCount.new(@instance, @prototype)
		$site.memcache.should_receive(:get).with(@relation.cache_key)
		demand(@relation)
	end
	
	it "should call find with count if it doesn't find a result in memcache" do
		@relation = RelationCount.new(@instance, @prototype)
		@prototype.target.should_receive(:find).with(:count, :example_find_options, 1, 2).and_return([])
		demand(@relation)
	end

	it "should memcache its result if it looks it up with find" do
		@relation = RelationCount.new(@instance, @prototype)
		$site.memcache.should_receive(:set).with(:example_cache_key,[])
		demand(@relation)
	end

	it "should be memcached" do
		RelationCount.should be_memcache
	end
