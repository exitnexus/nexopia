...
require "core/lib/template/template_tree_parser"

#Modified by Thomas Roy Feb 2007.
#Added support for getting the position of an element in its file.
...
module REXML
	...
	class TemplateDocument < Element
		...
		def initialize( source = nil, context = {} )
			...
			if source.kind_of? TemplateDocument
		...
		def TemplateDocument::parse_stream( source, listener )
			Parsers::StreamParser.new( source, listener ).parse
		end
		
		private
		def build( source )
			Parsers::TemplateTreeParser.new( source, self ).parse
		end
	end	
end
