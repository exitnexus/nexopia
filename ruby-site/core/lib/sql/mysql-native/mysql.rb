...
class Mysql
  ...
  def real_connect(host=nil, user=nil, passwd=nil, db=nil, port=nil, socket=nil, flag=nil)
    ...
    if (host == nil) and defined? UNIXSocket then
  ...
  def store_result(&block)
    ...
    if (!block)
      ...
      while rec = read_one_row(@field_count)
        res = Result::new self, @fields, @field_count, [rec]
        yield(res)
      end
      @fields = nil
      @affected_rows = 0
  ...
  def hash_password(password)
    return rb_hash_password(password)
  end

  def rb_hash_password(password)
    nr = 1345345333
    add = 7
    nr2 = 0x12345671
