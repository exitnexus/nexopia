...
module FriendFinder
	class WindowsLiveLogin
		...
		SETTINGS = {
			:beta => {	:app_id => "000000004801100E", :secret => "1Y61sVbnugxwm2iodfq4Z583ALBJqC7d", :security_algorithm => "wsignin1.0", :return_url => "http://www.beta.nexopia.com/friends/find/hotmail/return",		:policy_url => "http://www.beta.nexopia.com/privacy.php"},
			:stage => {	:app_id => "000000004801100F", :secret => "qsq3vesxL632dRNCD5yn1zgOlkiIBZhE", :security_algorithm => "wsignin1.0", :return_url => "http://www.stage.nexopia.com/friends/find/hotmail/return",	:policy_url => "http://www.stage.nexopia.com/privacy.php"},
			:live => {	:app_id => "0000000048011434", :secret => "iCXaaAC8XhbQSkgjgBRUJyiQZmGXGmQP", :security_algorithm => "wsignin1.0", :return_url => "http://www.nexopia.com/friends/find/hotmail/return", 				:policy_url => "http://www.nexopia.com/privacy.php"}
		}
	  ...
	  def debug(error)
	    ...
	    $log.info("Windows Live ID Authentication SDK #{error}", :debug)
	    nil 
	  end
	  ...
	  #####################################################################
	  def initialize()
			current_settings = WindowsLiveLogin::SETTINGS[$site.config_name.to_sym]
			if(current_settings.nil?())
				current_settings = WindowsLiveLogin::SETTINGS[:beta];
			end
	    self.force_delauth_nonprovisioned = true
	    self.appid = current_settings[:app_id]
	    self.secret = current_settings[:secret]
	    self.securityalgorithm = current_settings[:security_algorithm]
	    self.policyurl = current_settings[:policy_url]
	    self.returnurl = current_settings[:return_url]
	  end
	...
	#######################################################################
	  ...
	  # 'market' parameter.
	  #####################################################################
	  def getConsentUrl(offers = nil, context=nil, ru=nil, market=nil)
	    if(offers.nil?() || offers.empty?())
				offers = WindowsLiveLogin::OFFERS
			end
