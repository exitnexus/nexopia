...
$type = getREQval('type');
$id = getREQval('id');
$check = getREQval('check', 'array');

$types = array('both', 'inside', 'outside');
	...
	switch($action){
		case "Add News":	add($title,$news,$type);	break;
		case "Delete":		delete($check);	break;
		case "edit":		edit($id);	break;
		case "Update":		update($id,$title,$news,$type);	break;
	}
...
function edit($id){
	...
	$res = $db->prepare_query("SELECT title,text,type FROM news WHERE id = ?", $id);
	$line = $res->fetchrow();
	...
	echo "<tr><td class=body>Viewability:</td><td class=body><select class=body name=type>" . make_select_list( $types,$line["type"])."</select></td></tr>";
...
function update($id,$title,$news,$type){
	...
	$db->query("SELECT id, title, date, type, userid FROM news ORDER BY $sortt $sortd");
	...
	foreach($rows as $line){
		...
		echo "<td class=body><a class=body href=\"$_SERVER[PHP_SELF]?id=$line[id]&action=edit\">$line[title]</a></td>";
		echo "<td class=body>$line[type]</td>";
		echo "<td class=body>" . $users[$line['userid']]['username'] . "</td>";
		echo "<td class=body nowrap>" . userdate("m/d/y H:i",$line["date"]) . "</td>";
		echo "</tr>\n";
	}
	...
	echo "<tr><td class=body>Viewability:</td><td class=body><select class=body name=type>".make_select_list( $types)."</select></td></tr>";
