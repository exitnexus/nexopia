...
function add($title,$news,$type){
	...
	$fastdb->query("DELETE FROM cache WHERE name IN ('newsin','newsout')");

	$msgs->addMsg("News Added");
}

function delete($check){
	global $msgs, $db, $fastdb, $mods;
...
function edit($id){
	...
	echo "<tr><td class=body>Viewability:</td><td class=body><select class=body name=type>" . make_select_list( getEnumValues("news","type"),$line["type"])."</select></td></tr>";
...
function update($id,$title,$news,$type){
	...
	echo "  <td class=header><a class=header href=\"$PHP_SELF?sortd=" . ($sortt=="type" ? ($sortd=="ASC" ? "DESC" : "ASC") : $sortd). "&sortt=type\">Type</a>". ($sortt=="type" ? "&nbsp<img src=images/$sortd.png>" : "") ."</td>\n";
	...
	echo "  <td class=header><a class=header href=\"$PHP_SELF?sortd=" . ($sortt=="date" ? ($sortd=="ASC" ? "DESC" : "ASC") : $sortd). "&sortt=date\">Date</a>". ($sortt=="date" ? "&nbsp<img src=images/$sortd.png>" : "") ."</td>\n";
	echo "</tr>\n";
	while($line = $db->fetchrow($result))
		echo "<tr><td class=body><input type=checkbox name=check[] value=\"$line[id]\"></td><td class=body><a class=body href=\"$PHP_SELF?id=$line[id]&action=edit\">$line[title]</a></td><td class=body>$line[type]</td><td class=body>$line[username]</td><td class=body nowrap>" . userdate("m/d/y H:i",$line["date"]) . "</td></tr>\n";
	...
	echo "<tr><td class=body>Viewability:</td><td class=body><select class=body name=type>".make_select_list( getEnumValues("news","type"))."</select></td></tr>";
	echo "<tr><td class=body>Text:  </td><td class=body><textarea class=body name=news cols=33 rows=8></textarea></td></tr>\n";
