...
class Template
{
    ...
    function Template($filepath, $parsed = true)
    {
        ...
        if(!$this->tmpl_str)
        {
            die ("File ".PARSED_FILES_PATH."$filepath.parsed.php is empty");
        }
    ...
    function setMultiple($vars)
    {
        foreach($vars as $key => $value)
            $this->set($this->tmpl_name_prefix ."_".$key, $value);
    }
  ...
  //returns a string of the template that can be echoed.
    function parse($str)
    {
        ...
        while($i < $strlen)
        {
            ...
            {    
                ...
                if(!$is_php_comment && $current_char == '/' && $str{$i-1} == '/')
                {
                    $is_php_comment = true;
                    $comment = 1;
                }
     ...
     */   
    function process_var($tag, &$is_output)
    {
        $tag = trim($tag);
        $php_var = "";
        $is_output = true;
        ...
        while(++$i < strlen($tag))
        {
            ...
            elseif(!$var_started && strpos($validprefixchar,$tag{$i}) !== false && strpos($validvarfirstchar, $tag{$i+1}) !== false)
            {
    ...
    function process_function($tag, $var)
    {
        ...
        if($func_tmpl != null)
        {
            ...
            for( $i = 1; $i < count($function_array); $i++)
            {
                ...
                {
                    $variable = process_var($function_array[$i]);
                    if($variable == null)
                    {
                        $func_str = null;
                        break;
    ...
    function process_loop($tag)
    {
        ...
        if($as_pos !== false)
        {
            ...
            {   
                ...
                {
                    $value_var = $this->process_var(substr( $loop_cond, $arrow_pos + 2, $comma_pos - ($arrow_pos + 2)));
