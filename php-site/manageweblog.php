...
function addJournalEntry($id = 0, $title = "", $msg = "", $scope = "", $preview = false){
	global $PHP_SELF, $db, $possibleScope, $uid;
...
function listJournal(){
	global $page, $db, $action, $uid, $userData, $config, $possibleScope, $mod, $PHP_SELF;
	...
	foreach($rows as $line){
		...
		echo "<td class=body><a class=body href=$PHP_SELF?action=edit&id=$line[id]&uid=$uid><img src=$config[imageloc]edit.gif border=0></a>";
		echo "<a class=body href=$PHP_SELF?action=delete&id=$line[id]&uid=$uid><img src=$config[imageloc]delete.gif border=0></a></td>";
		echo "</tr>";
	}
