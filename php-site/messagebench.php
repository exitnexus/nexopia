...
function listMessages(){
	global $config, $benchdb, $uid, $bench;

	$page = 0;

	$folder = MSG_INBOX;

	$query = "SELECT " . ($bench['getnumpages'] ? "SQL_CALC_FOUND_ROWS " : "") . "id, userid, folder, `to`, toname, `from`, fromname, date, mark, status, subject FROM msgs WHERE userid = # /*&& folder = # */ ORDER BY date DESC LIMIT #, #";
...
function viewMsg($id){
	...
	if($msg['status']=='new'){
		$benchdb->prepare_query($msg['to'], "UPDATE msgs SET status = 'read' WHERE userid = # && id IN (#)", $msg['to'], $id);
		$benchdb->prepare_query($msg['from'], "UPDATE msgs SET status = 'read' WHERE userid = # && id IN (#)", $msg['from'], $msg['othermsgid']);
	}
...
function getMsg($uid, $ids, $cached = false){ //cached version has an outdated status, and may not show the right folder
	...
	if(count($missingids)){
		$res = $benchdb->prepare_query($uid, "SELECT id, userid, `to`, toname, `from`, fromname, date, status, subject, replyto, folder, othermsgid FROM msgs WHERE userid = # && id IN (#)", $uid, $missingids);
	...
	if(count($missingids)){
		$res = $benchdb->prepare_query($uid, "SELECT id, msg, html FROM msgtext WHERE userid = # && id IN (#)", $uid, $missingids);
...
function deliverMsg($toid, $subject, $message, $replyto = 0, $fromname = false, $fromid = false, $ignorable = true, $html = false){
	global $emaildomain, $config, $benchdb, $cache;
	...
	if($replyto){
		$res = $benchdb->prepare_query($toid, "SELECT othermsgid FROM msgs WHERE userid = # && id = #", $toid, $replyto);
	...
	foreach($tousers as $toid => $toname){
		...
		$benchdb->prepare_query($toid, "INSERT INTO msgtext SET userid = #, id = #, msg = ?, date = #, html = ?", $toid, $firstmsgid, $nmsg, $time, $html);
		$cache->put("msgtext-$toid-$firstmsgid", $msgtext, 86400);

		$benchdb->prepare_query($toid, "INSERT INTO msgs SET userid = #, id = #, folder = #, otheruserid = #, `to` = #, toname = ?, `from` = #, fromname = ?, date = #, status = 'new', subject = ?, replyto = #, othermsgid = #",
							$toid, $firstmsgid, MSG_INBOX, $fromid, $toid, $toname, $fromid, $fromname, $time, $nsubject, $otherreply, $secondmsgid);


		if($fromid){
			$benchdb->prepare_query($fromid, "INSERT INTO msgtext SET userid = #, id = #, msg = ?, date = #, html = ?", $fromid, $secondmsgid, $nmsg, $time, $html);
			$cache->put("msgtext-$fromid-$secondmsgid", $msgtext, 86400);

			$benchdb->prepare_query($fromid, "INSERT INTO msgs SET userid = #, id = #, folder = #, otheruserid = #, `to` = #, toname = ?, `from` = #, fromname = ?, date = #, status = 'new', subject = ?, replyto = #, othermsgid = #",
								$fromid, $secondmsgid, MSG_SENT, $toid, $toid, $toname, $fromid, $fromname, $time, $nsubject, $replyto, $firstmsgid);
		}
...
//					$firstmsgid, $toid, $toname, $fromid, $fromname, $time, $nsubject, $nmsg);
	...
	if($otherreply){
		$benchdb->prepare_query($toid, "UPDATE msgs SET status='replied' WHERE userid = # && id = #", $toid, $replyto);
		$benchdb->prepare_query($fromid, "UPDATE msgs SET status='replied' WHERE userid = # && id = #", $fromid, $otherreply);
	}
