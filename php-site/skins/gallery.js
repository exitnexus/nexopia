...
function picloaded(picid, childdoc, container)
{
	...
	var prevnextfull = wrapslice(thumbs, prevnextidx);
...
function setuplinks(picid, picidx)
{
	...
	var src = 'http://img0.img.nexopia.com' + '/gallery/' + userid + '/' + picid + '.jpg';
...
function submitfile(legacy, autosubmit)
{
	...
	try {
		...
		if (autosubmit)
		{
			formobj.userfile.onchange = function() { setTimeout('submitfile();', 100); };
		}
