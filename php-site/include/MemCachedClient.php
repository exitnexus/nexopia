...
class MemCachedClient
{
	...
	function get_multi($keys)
	{
		...
		foreach($keys as $k)
		{
			...
			if($sock)
			{
				...
				if(!isset($sock_keys[$sock]) || !is_array($sock_keys[$sock]))
	 ...
	 * @param int $value the amount to increment the key by
	...
	function _set($cmdname, $key, $val, $exptime = 0)
	{
		...
		if (($this->compress > 0) && (strlen($val) > $this->compress)) {
	...
	function _load_items($sock, &$val, $sock_keys)
	{
		...
		while($line = socket_read($sock, MC_BUFFER_SZ, PHP_BINARY_READ))
		{
			...
			if($len == 0)
			{
				...
				if(isset($matches[1]) && isset($matches[2]) && isset($matches[3]) && is_string($matches[1]) && is_numeric($matches[2]) && is_numeric($matches[3]))
				{
					$rk = $matches[1];
					$flags = $matches[2];
					$len = $matches[3];
			...
			if($bytes_read == ($len + 7))
			{
				...
				if($end == "END")
				{
					...
					foreach($sock_keys as $sk)
					{
						if(!isset($val[$sk]) || !isset($flags_array[$sk]))
							continue;
