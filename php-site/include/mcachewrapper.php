...
//wrapper class for the mcache client library. Makes it interface compatible with memcached-client
...
class memcached {
	...
	function add($key, $val, $exp = 0){
		$this->stats['add']++;
		return $this->memcache->add($key, $val, $exp);
	}
	...
	function replace($key, $value, $exp=0){
		$this->stats['replace']++;
		return $this->memcache->replace($key, $value, $exp);
	}

	function set($key, $value, $exp=0){
		$this->stats['set']++;
		return $this->memcache->set($key, $value, $exp);
	}
