...
//bannersize defines
define("BANNER_BANNER", 	"468z60");
define("BANNER_LEADERBOARD","728x90");
define("BANNER_BIGBOX", 	"300x250");
define("BANNER_SKY120", 	"120x600");
define("BANNER_SKY160", 	"160x600");
define("BANNER_BUTTON60",	"120x60");
...
class banner{
	...
	var $bannersizes = array("728x90", "468x60", "120x60", "120x240", "120x600", "300x250");
...
//could do this without the join (grab statsid in select above, update banners.views hourly), but the resulting race condition probably isn't worth it.
...
//		$this->updateBannerHits($id);
		...
		switch($type){
			...
			case "html":
				if(strpos($alt, "%") !== false){
					global $userData;
					$rand = rand();
					$page = ucfirst(substr($_SERVER['PHP_SELF'], 1, -4));
	...
	function calculateWeightings($size = false){
		global $cache, $messaging, $debuginfousers;
		if(!$size){
		...
		if($viewsremaining > 0)
			$messaging->deliverMsg($debuginfousers[0], "Empty Impressions", "Size = $size\nTotal: $totalviews\nRemaining: $viewsremaining", 0, "Nexopia", 0);
