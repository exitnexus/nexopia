...
function edit($filename){
	global $opendirperms,$msgs,$basedir,$opendir,$sortt,$sortd;
...
function view($filename){
	...
	switch(getFileType($filename)){
		...
		case "image":
			...
			echo "<a class=body href=$_SERVER[PHP_SELF]?opendir=$opendir><img src=\"$baseuserdir$opendir$filename\" border=0></a>";
...
function browse($opendir){
	global $basedir,$baseuserdir,$userData,$opendirperms, $sortt, $sortd;
	...
	echo "<tr>\n";
	echo "	<td class=header width=20><img src=/images/delete.gif alt=Delete></td>";
	echo "	<td class=header width=20><img src=/images/rename.gif alt=Rename></td>";
	echo "	<td class=header width=20><img src=/images/down.png alt=Download></td>";
	echo "	<td class=header width=20><img src=/images/edit.gif alt=Edit></td>";

	echo "	<td class=header><a class=header href=\"$_SERVER[PHP_SELF]?opendir=$opendir&sortd=" . ($sortt=="name" ? ($sortd=="ASC" ? "DESC" : "ASC") : $sortd). "&sortt=name\">Name</a>". ($sortt=="name" ? "&nbsp<img src=/images/$sortd.png>" : "") ."</td>\n";
	echo "	<td class=header align=right><a class=header href=\"$_SERVER[PHP_SELF]?opendir=$opendir&sortd=" . ($sortt=="size" ? ($sortd=="ASC" ? "DESC" : "ASC") : $sortd). "&sortt=size\">Size</a>". ($sortt=="size" ? "&nbsp<img src=/images/$sortd.png>" : "") ."</td>\n";
	echo "	<td class=header align=right><a class=header href=\"$_SERVER[PHP_SELF]?opendir=$opendir&sortd=" . ($sortt=="date" ? ($sortd=="ASC" ? "DESC" : "ASC") : $sortd). "&sortt=date\">Date</a>". ($sortt=="date" ? "&nbsp<img src=/images/$sortd.png>" : "") ."</td>\n";
	echo "</tr>\n";
...
//delete
				echo "<td class=body>";
				if($perms['write']=='y')
					echo "<a class=body href=\"javascript:confirmLink('$_SERVER[PHP_SELF]?opendir=$opendir&action=delete&filename=$file[filename]','delete this directory')\"><img src=/images/delete.gif border=0 alt=Delete></a> ";
				echo "</td>";
//rename
				echo "<td class=body>";
				if($perms['write']=='y')
					echo "<a class=body href=\"javascript: if(name = prompt('Rename to what?','$file[filename]')) location.href= '$_SERVER[PHP_SELF]?opendir=$opendir&action=rename&filename=$file[filename]&new=' + name\"><img src=/images/rename.gif border=0 alt=Rename></a> ";
				echo "</td>";
				...
				echo "[&nbsp;";
				if($perms['list'])
					echo "<a class=body href=\"$_SERVER[PHP_SELF]?opendir=$opendir$file[filename]/&sortd=$sortd&sortt=$sortt\">";
				echo $file['filename'];
				if($perms['list'])
					echo "</a>";
				echo "&nbsp;]";
...
//delete
				...
				if($opendirperms['write']=='y')
					echo "<a class=body href=\"javascript:confirmLink('$_SERVER[PHP_SELF]?opendir=$opendir&action=delete&filename=$file[filename]','delete this file')\"><img src=/images/delete.gif border=0 alt=Delete></a> ";
				echo "</td>";
//rename
				echo "<td class=body>";
...
//down
				...
				if($opendirperms['read']=='y' && (getFileType($file['filename'])!='php' || $opendirperms['readphp']=='y'))
					echo "<a class=body href=\"$_SERVER[PHP_SELF]?opendir=$opendir&action=download&filename=$file[filename]\"><img src=/images/down.png border=0 alt=Download></a> ";
				echo "</td>";
//edit
				echo "<td class=body>";
				if($opendirperms['read']=='y' && ($file['filetype']=='text' || ($file['filetype']=='php' && $opendirperms['writephp']=='y')))
					echo "<a class=body href=\"$_SERVER[PHP_SELF]?opendir=$opendir&action=edit&filename=$file[filename]\"><img src=/images/edit.gif border=0 alt=Edit></a>";
				echo "</td>";
				...
				if($opendirperms['read']=='n' || getFileType($file['filename'])=='unknown' || ($opendirperms['readphp']=='n' && getFileType($file['filename'])=='php'))
					...
					echo "<a class=body href=\"$_SERVER[PHP_SELF]?opendir=$opendir&action=view&filename=$file[filename]\">$file[filename]</a>";
			}
	...
	echo "<tr><td class=header></td><td class=header></td><td class=header></td><td class=header></td><td class=header>$opendir</td><td class=header align=right>" . number_format($totalsize) . " KB</td><td class=header align=right></td></tr>\n";
	echo "</table>";
	...
	if($opendirperms['write']=='y'){
		...
		echo "<input type=hidden name=sortt value=$sortt>";
		echo "<input type=hidden name=sortd value=$sortd>";
		echo "<tr><td class=header colspan=2>Create New Text File</td></tr>";
