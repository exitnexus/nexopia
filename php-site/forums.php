<?
	...
	if($userData['loggedIn'])
		$forums->db->prepare_query("SELECT forums.*, IF( forumupdated.time IS NULL, 1, forums.time > forumupdated.time ) AS new FROM forums LEFT JOIN forumupdated on forums.id = forumupdated.forumid && forumupdated.userid = ? WHERE official='y'", $userData['userid']);
	else
		$forums->db->query("SELECT *, 0 as new FROM forums WHERE official='y'");
	...
	foreach($parentdata[0] as $realm){
		echo "<tr><td class=header colspan=4><b>" . $forumdata[$realm]['name'] . "</b></td></tr>\n";
