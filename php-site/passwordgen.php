...
function binl2hex(binarray){
	var hex_tab = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!@^*_+-#:,.|";
	var str = '';
	for(var i = 0;i < binarray.length;i++){
		num = Math.abs(binarray[i]);
		for(var j=0;j<2;j++){
			str += hex_tab.charAt(num % hex_tab.length);
			num /= hex_tab.length;
		}
	}
	return str;
...
function doIt(){
	...
	var i=0, j=0, p=hex_md5(master+':'+host).substr(0,8), F=document.forms, found=false;
...
echo "<a href=\"javascript: " . str_replace("+","%20",urlencode(str_replace(array("\n","\r","\t"),"", $script))) . "\">Generate Password</a>";
