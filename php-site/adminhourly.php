 ...
 *
 * This module may also be run from RAP/the PPQ, in which case it will run in non-interactive mode.
 * END SYNOPSIS */
...
if (! getenv('ADMINHOURLY_ENABLED'))
{
	echo "ADMINHOURLY_ENABLED environment variable is not set.\n";
	echo "\n";
	usage();
	exit;
}
...
/* : okay now deal with running in non-interactive mode */
	...
	/* : process nightly stats here (at 2:00am) */
	/* general.lib.php sets the default timezone to UTC, so in order to get actual local
	 * time we cannot depend on the default localtime()/time()/date() functions. */
	if (date_create("now", new DateTimeZone("Canada/Mountain"))->format("H") == 2) {
		/* : perform daily tasks */
		daily();

		/* : optimize the database */
		/* optimize(); <-- moved to the mysql daily/weekly/monthly maintenance scripts on the database servers themselves. */
	}
...
function daily () {
	...
	$archive->createTable(); //potentially create next months table
	...
	echo $timer->lap("clear blog hit history after a week");
	$usersdb->repeatquery($usersdb->prepare("DELETE FROM blogviews WHERE time <= ?", $newtime - 86400*7));
...
function usage () {
	...
	echo "Usage: ADMINHOURLY_ENABLED=1 $scriptName <function> <function> <function> ...\n";
	echo "\n";
